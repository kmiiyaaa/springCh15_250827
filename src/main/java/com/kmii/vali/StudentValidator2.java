...
public class StudentValidator2 implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		if(id.strip().isEmpty() || id == null ){ // 두 조건중 하나라도 참이면 erorr
			...
			errors.rejectValue("id", "id가 공란입니다.");  // ( 에러가 발생한필드 이름, 에러코드(메시지)
			
		}
		
//		if(pw.strip().isEmpty() || pw == null ){ // 두 조건중 하나라도 참이면 erorr
//			System.out.println("에러가 발생된 비밀번호  " + pw);  // 
//			errors.rejectValue("pw", "비밀번호가 공란입니다.");  // ( 에러가 발생한필드 이름, 에러코드(메시지)
//			
//		}

		// 위 비밀번호 오류를 더 줄여서 한다면?
		...
		if(age>19 || age<0) { // 나이가 19세 초과일 경우 학생회원 자격 없음error
			errors.rejectValue("age","19세 초과, 가입이 불가능 합니다.");  //( 에러가 발생한필드 이름, 에러코드(메시지)
